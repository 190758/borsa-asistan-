...
# Hisse Seçimi
hisse_listesi = ["FROTO.IS", "ISMEN.IS", "ANHYT.IS", "ARDYZ.IS", "ALTNY.IS", "YEOTK.IS"]
secilen_hisse = st.selectbox("Analiz Edilecek Hisseyi Seçin:", hisse_listesi)

if st.button("Olasılık Grafiğini Oluştur"):
    with st.spinner("Borsa İstanbul verileri çekiliyor ve olasılık hesaplanıyor..."):
        ...
        if df is not None and not df.empty and len(df) > 10:
            ...
            with col1:
                st.metric("30 Gün Destek (Düzeltme)", f"{alt_bant[29]:.2f} TL")
                st.metric("30 Gün Hedef (Direnç)", f"{ust_bant[29]:.2f} TL")
                
            with col2:
                st.metric("60 Gün Destek (Düzeltme)", f"{alt_bant[59]:.2f} TL")
                st.metric("60 Gün Hedef (Direnç)", f"{ust_bant[59]:.2f} TL")
                
            with col3:
                st.metric("90 Gün Destek (Düzeltme)", f"{alt_bant[89]:.2f} TL")
                st.metric("90 Gün Hedef (Direnç)", f"{ust_bant[89]:.2f} TL")
            ...
            st.error("Veri çekme sırasında bağlantı gecikmesi yaşandı. Lütfen 'Olasılık Grafiğini Oluştur' butonuna tekrar basın.")
